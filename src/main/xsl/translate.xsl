...
<xsl:stylesheet 
	...
	<xsl:param name="dictionary" select="document('../xml/cards/dictionary.xml')/*" as="element()?" />
	<xsl:param name="target-language" select="'en-us'" as="xs:string" />
	...
	<xsl:template match="game">
		...
		<xsl:variable name="entries" as="element()*">
			<xsl:if test="$source-language != '' and $target-language != ''">
				<xsl:copy-of select="$dictionary//entry[term/@xml:lang = $source-language and term/@xml:lang = $target-language]" />
			</xsl:if>
		</xsl:variable>
	...
	<xsl:template match="game | deck | card" mode="translate">		
